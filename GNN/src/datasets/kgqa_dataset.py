...
class KGQADataset(torch.utils.data.Dataset):
    def __init__(
        ...
        self.loaded_entity_to_idx, self.loaded_edge_index, self.loaded_relations = (
            self.load_data_json(path_to_idxes)
        )
        self.data = self.create_data_object(
            self.loaded_edge_index, self.loaded_relations, self.loaded_entity_to_idx
        )

        # load the questions and answers
        self.df = pd.read_csv(
            path_to_qa, sep="\t", header=None, names=["question", "answer"]
        )
    ...
    def create_data_object(self, edge_index, relations, entity_to_idx):
        ...
        undirected_edge_attr = torch.cat(
            [edge_attr, edge_attr], dim=0
        )  # comment out if want directed

        return Data(
            edge_index=undirected_edge_index,
            edge_attr=undirected_edge_attr,
    ...
    def load_node2vec_embeddings(self, file_path, embedding_dim=64):
        ...
        with open(file_path, "r") as f:
            ...
            for line in f:
                ...
                entity = " ".join(
                    parts[:-embedding_dim]
                )  # Join all words before the embedding dimensions
                embedding = list(
                    map(float, parts[-embedding_dim:])
                )  # Convert last parts to float
    ...
    def get_k_hop_subgraph(self, node_idx):
        ...
        node_map = {
            original_idx.item(): new_idx for new_idx, original_idx in enumerate(subset)
        }
    ...
    def get_node_embeddings(self, node_map, embedding_dim=64, random_init=False):
        ...
        """
        embeddings = [[0.0] * embedding_dim] * len(
            node_map

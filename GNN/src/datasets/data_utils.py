...
def data_loader(dataset_name, encoding_model, config):

    dataset = KGQADataset(
        encoding_model,
        path_to_node_embed=config["node_embed"],
        path_to_idxes=config["idxes"],
        path_to_qa=config[f"{dataset_name}_qa_data"],
        path_to_ans_types=config[f"{dataset_name}_ans_types"],
        train=True,
        k=config["num_hops"],
    )
    ...
    sub_train_data = torch.utils.data.Subset(
        dataset,
        list(range(config[dataset_name]["start_idx"], config[dataset_name]["end_idx"])),
    )
